...
import { getLocalBlogPostRecords, getLocalBlogPosts } from './local-posts'
import {
  fromUniqueListingFields,
  isDuplicatePost,
  rankedToBlogPost,
  relatedFromPosts,
  withUniqueListingFields,
} from './map-post'
...
export async function getLiveRankedBlogPosts(
  ...
  try {
    ...
    const localKeys = local.map((p) => ({ title: p.title, slug: p.slug }))
    const reservedCovers = new Set(local.map((p) => p.coverImage))
    const resolved = await Promise.all(
      candidates.map(async (item) => {
        if (isDuplicatePost(item.title, localKeys)) return null
...
export async function getPublishedBlogPosts(): Promise<BlogPost[]> {
  const local = getLocalBlogPostRecords()
  ...
  const merged = [
    ...local,
    ...ranked
      .filter((p) => !taken.has(p.slug) && !isDuplicatePost(p.title, localKeys))
      .map(rankedToBlogPost),
  ]
  const uniqued = ensureUniquePublishDates(ensureUniqueCoverImages(withUniqueListingFields(merged)))
  return fromUniqueListingFields(uniqued)
}
...
export async function getRelatedPublishedPosts(slug: string, limit = 3): Promise<BlogPost[]> {
  const posts = await getPublishedBlogPosts()
  return relatedFromPosts(posts, slug, limit)
}
